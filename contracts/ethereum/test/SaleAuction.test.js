...
contract('MockSaleAuction', accounts => {
  ...
  describe('#bid()', () => {
    ...
    it('reverts when one who is not buyer tries to bid', async () => {
      await shouldFail.reverting(SaleAuction.bid(ghostID, {from: owner, value: web3.utils.toWei("1000", "wei")}));
    });

    it('reverts when amount of bidding is too low', async () => {
      await shouldFail.reverting(SaleAuction.bid(ghostID, {from: buyer, value: web3.utils.toWei("900", "wei")}));
    });

    it('successfully buyer bids', async () => {
      await SaleAuction.bid(ghostID, {from: buyer, value: web3.utils.toWei("1000", "wei")});

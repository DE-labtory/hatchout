...
contract SaleAuction is AuctionBase {
    ...
    function createAuction(uint256 _tokenId, address payable _seller, address payable _buyer, uint256 _amount) external {
        ...
        // saleAuction's type is 0
        _addAuction(_tokenId, auction, 0);
    }
    ...
    function bid(uint256 _tokenId) external payable {
        ...
        require(auction.maxPrice >= msg.value);

        // TODO: send ether (max price of auction) to owner from buyer
        uint256 fee = msg.value;

        if (fee > auction.maxPrice) {
            uint256 proceeds = fee - auction.maxPrice;

            msg.sender.transfer(proceeds);
        }

        auction.seller.transfer(auction.maxPrice);

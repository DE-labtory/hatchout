...
module.exports = function(_contractAddr = '') {
  abi = fs.readFileSync(path.join(__dirname, '../abi/', 'MockHatchOut.json'), {'encoding': 'utf8'});
  ...
  return {
    ...
    methods: {
      ...
      isSaleAuction: function(_tokenId, ) {
        return contract.methods.isSaleAuction(_tokenId, ).call();
      },
      tokensOfOwner: function(_owner, ) {
        return contract.methods.tokensOfOwner(_owner, ).call();
      },
      getSpecialAuctionAddress: function() {
        return contract.methods.getSpecialAuctionAddress().call();
      },
      symbol: function() {
      ...
      setAuctionAddresses: function(_specialAuctionContractAddress, _saleAuctionContractAddress, options) {
        const txData = contract.methods.setAuctionAddresses(_specialAuctionContractAddress, _saleAuctionContractAddress, ).encodeABI();
        options = {
          ...options,
          data: txData,
          platform: platform
